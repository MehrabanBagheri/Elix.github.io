...
let Numbers=()=>{
        animate(text1, 0, 870, 12000);
        animate(text2, 0, 970, 12000);
        animate(text3, 0, 400, 12000);
    ...
    $.ajax({
        ...
        success:function (response) {
            response.map(function (elem,index) {
                itemPosts+=`
                        <div class="itemPosts">
                        <div><h1>${elem.date} <br> ${elem.month}</h1></div>
                        <img src="${elem.image}" alt=""style="">
                        <h4><span>${elem.title}</span> -${elem.comments}Comments</h4>
                        <h5>${elem.description}</h5>
                        <a href="../blog-detail.html">Read More</a>
                        </div>
            `})
            document.getElementById("Posts").innerHTML=itemPosts
        }
    })
}
document.addEventListener("DOMContentLoaded",ajaxLoadedPosts)
